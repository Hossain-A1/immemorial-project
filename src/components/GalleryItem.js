...
const GalleryItem = ({image}) => {
  ...
  return (
    <div className="gallery-item">
      <h1 className="images-title" ref={galleryTitleRef}>{image.title}</h1>
      <p className="gallery-item-category" ref={galleryCategoryRef}>{image.category}</p>
      <div className="gallery-item-img" ref={galleryImageRef} style={{backgroundImage: `url(${image.src})`}}></div>
    </div>
  )
}
